---
name: dietmcp
description: Route all MCP server calls through dietmcp CLI to reduce context window bloat by 90%+. Use this skill whenever you need to interact with ANY MCP server -- documentation lookup, database queries, GitHub operations, file operations, web search, fetching URLs, or any external data source. Activate when you see native mcp__* tools, when a user asks you to "look up docs", "check the API", "query the database", "search GitHub", "fetch a URL", or whenever you would otherwise use a native MCP tool call. Also use when the user mentions dietmcp, context window optimization, or MCP servers directly. If in doubt about whether to route through dietmcp, do it -- the context savings compound across every call in a session.
---
